...
class Process:
    ...
    def __init__(self, data_path=None, filename=None):
        ...
        self.prefix = "landmark_aligned_face."
        ...
        for ind, folder in enumerate(all_folders):
            ...
            for i in tqdm(range(len(folder))):
                img_path = self.data_path + \
                    os.path.join("/aligned", folder[i][0])
    ...
    def split_data_from_dirs(self, data, ages, genders, split):
        print("Number of images in the training data: {}".format(
            str(data.shape[0])))
        print("Ages/Genders: {}".format(str(ages.shape)))
        split_factor = int(np.floor(split * data.shape[0]))
        ...
        print("Training Gender labels shape: {}".format(
            str(y_train_gender.shape)))
    ...
    def helper(self):
        ...
        split = 0.95
        x_train, x_test, y_train_age, y_test_age, y_train_gender, y_test_gender = self.split_data_from_dirs(
            data, ages, genders, split)
        self.generate_h5(x_train, x_test, y_train_age,
                         y_test_age, y_train_gender, y_test_gender)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Use this script to process dataset.')
    parser.add_argument('-p', '--path', type=str, required=True, default=os.path.join(
        os.getenv("HOME"), "data/adience"), help='Path to raw dataset file to be processed.')
    parser.add_argument('-o', '--save', type=str, required=True, default=os.path.join(
        os.getenv("HOME"), "data/adience/adience.h5"), help='Path to save the .h5 file')
